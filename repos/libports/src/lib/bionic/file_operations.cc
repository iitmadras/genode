...
{
	//PERR("file_operations.cc 	libc_fd_to_fd %d %s %d",libc_fd,func_name,file_descriptor_allocator()->find_by_libc_fd(libc_fd));
	File_descriptor *fd = file_descriptor_allocator()->find_by_libc_fd(libc_fd);
	//PERR("file_operations.cc  	libc_fd_to_fd %d %s paramesh  %s",libc_fd,func_name,fd->libc_fd);
	if (!fd)
...
extern "C" int __connect(int libc_fd, const struct sockaddr *addr,
                        socklen_t addrlen)
{
    return connect(libc_fd, addr, addrlen);
}
...
extern "C" int ioctl(int libc_fd, int request, ...) {

	va_list ap;
	va_start(ap, request);
	
	FD_FUNC_WRAPPER(ioctl, libc_fd, request, va_arg(ap, char *)); 
	va_end(ap);
	}

extern "C" int __ioctl(int libc_fd, int request, void *argp){
	FD_FUNC_WRAPPER(ioctl, libc_fd, request, (char *)argp); 
...
extern "C" int pipe2(int pipefd[2], int flags){
	...
	// if flags=0 pipe2 acts as pipe
	if(flags==0) pipe(pipefd);
	// else 
	else return 0;
}
...
extern "C" int __socket(int domain, int type, int protocol)
{
	return socket(domain, type, protocol);
}
...
extern "C" void *memcpy (void *__restrict __dest, const void *__restrict __src,
                     size_t __n){
                     return Genode::memcpy(__dest,__src,__n);
